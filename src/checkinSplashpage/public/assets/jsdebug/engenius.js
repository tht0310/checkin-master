

function jwifi_login_hotspot(url) {
    if (!window.jwifi_impression && typeof window.jwifi_impUrl == 'string') {
        var img = new Image();
        img.src =  window.jwifi_impUrl;
        img.onerror = function() { throw "Impression can't loaded on " + navigator.userAgent; }
        img.onload = function() { window.jwifi_impression = true; }
    }
    

    var redirectUrl = jwifi_friendly_clickUrl;
    if (typeof url == "string") {
        redirectUrl += "&landingpage=" + url;
    }

    if (typeof localStorage !== "undefined") {
        localStorage.setItem("redirectUrl", redirectUrl);
    }

    var url = new URL(decodeURIComponent(jwifi_getParam('actionurl')));

    var actionurl = "https://falcon.service.engenius.ai/captive-portal/auth?" 
    var loginurl = url.searchParams.get("loginurl") || decodeURIComponent(jwifi_getParam('actionurl').split("?")[0]);
    
    actionurl = loginurl;
    actionurl += "?mac=" + mac_device.replace(/-/g, ':');
    actionurl += "&called=" + mac_ap.replace(/-/g, ':');
    actionurl += "&userurl=" + encodeURIComponent("http://becamex.com.vn")

    var f = document.createElement('form');
    f.name = 'frmEngenius';
    f.method = "POST";
    f.action = actionurl;

    var e1 = document.createElement("input");
    e1.value = username;
    e1.name = "username";
    e1.type = "hidden";
    f.appendChild(e1);

    var e2 = document.createElement("input");
    e2.value = password;
    e2.name = "password";
    e2.type = "hidden";
    f.appendChild(e2);

    var n = document.getElementsByTagName('head')[0].appendChild(f);

    if (typeof jwifi_logHit == "function") {
        jwifi_logHit(function () {
            n.submit(); setTimeout(function () { document.getElementsByTagName('head')[0].removeChild(n) }, 2000);
        });
    }
    else {
        n.submit(); setTimeout(function () { document.getElementsByTagName('head')[0].removeChild(n) }, 2000);
    }
}